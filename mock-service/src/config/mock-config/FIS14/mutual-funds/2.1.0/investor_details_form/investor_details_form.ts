...
export class MockInvestorDetailsFormClass extends MockAction {
	name(): string {
		return "investor_details_form";
	}
	get description(): string {
		return "Mock for investor_details_form";
	}
	...
	async validate(
		...
		const formLink = sessionData["investor_details_form"];
		if (!formLink) {
	...
	override async __forceSaveData(
		...
		return {
			...sessionData,
			investor_details_form: resolveFormActions(formLink, formData),
		};
	}
	...
	get saveData(): saveType {
		return { "save-data": { investor_details_form: "investor_details_form" } };
	}
	get defaultData(): any {
		return {};
	}
