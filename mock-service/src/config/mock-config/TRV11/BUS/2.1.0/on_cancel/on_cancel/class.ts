...
export class MockOnCancelBusTech_Soft210Class extends MockAction {
  get saveData(): saveType {
    return yaml.load(
      readFileSync(path.resolve(__dirname, "../save-data.yaml"), "utf8")
    ) as saveType;
  ...
  name(): string {
    return "on_cancel_tech_soft_BUS_210";
  }
  generator(existingPayload: any, sessionData: SessionData): Promise<any> {
    return onCancelGenerator(existingPayload, sessionData, true);
  }
  get description(): string {
    return "Mock for on_cancel_tech_soft_BUS_210";
  }
  async validate(
    targetPayload: any,
    sessionData: SessionData
