...
export class MockCancelTechSoftBus210Class extends MockAction {
  get saveData(): saveType {
    return yaml.load(
      readFileSync(path.resolve(__dirname, "../save-data.yaml"), "utf8")
    ) as saveType;
  ...
  name(): string {
    return "cancel_tech_soft_BUS_210";
  }
  generator(existingPayload: any, sessionData: SessionData): Promise<any> {
    return cancelTechGenerator(existingPayload, sessionData, true);
  }
  get description(): string {
    return "Mock for cancel_tech_soft_BUS_210";
  }
  async validate(
    targetPayload: any,
    sessionData: SessionData
